...
def create_example_from(question,paragraph):
    doc_tokens = []
    char_to_word_offset = []
    prev_is_whitespace = True
    for c in paragraph:
...
def extract_result(all_examples, all_features, all_results, n_best_size,
...
class BertSquad(object):
    ...
    def load(self):
        ...
        parser.add_argument('--do_lower_case', action="store_true", default=False, help="Lowercase the input")
        ...
        self.args = parser.parse_args(['--bert_model=bert-base-cased'
        ,'--output_dir=./output','--do_predict','--predict_file=./squad/dev-v1.1.json'])
    ...
    def predict(self,paragraph,question):
        eval_examples = create_example_from(question,paragraph)
        # self.device = self.device
        # eval_examples = read_squad_examples(
        #     input_file=self.args.predict_file, is_training=False)
        ...
        logger.info("  Batch size = %d", self.args.predict_batch_size)
        ...
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=self.args.predict_batch_size)
        ...
        #                   output_nbest_file, self.args.verbose_logging)
...
if __name__ == "__main__":
    ...
    ques = 'Which NFL team represented the AFC at Super Bowl 50?'
    bs = BertSquad()
    bs.load()
    result = bs.predict(para,ques)
